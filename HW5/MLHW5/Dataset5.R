...
# Class attribute is column 34 ie V34
...
for(i in 1:10) 
{
    ...
    method <- "kNN"
    # Model
    knn.model <- kknn(formula = formula(V35~.),train=trainingData,test=testData,k = 3,distance=1)
    # Predict
    fit <- fitted(knn.model)
    tab3<-table(testData$V35, fit)
    ...
    bagTrainModel <- bagging(V35~., data=trainingData, control=control, nbagg=10, coob=TRUE)
    # Predict
    ...
    # Model 
    rFTrainmodel <- randomForest(as.factor(V35) ~ ., data=trainingData, ntree=500, keep.forest=TRUE, importance=TRUE)
    # Predict
    ...
    method <- "Boosting"
    # Model
    boostingTrainModel <- ada(V35 ~ ., data = trainingData, type="discrete")
    # Predict
    ...
    method <- "GBM Boosting"
    # Model
    trainingData$V35 = ifelse(trainingData$V35 == "2", 1, 0)
    testData$V35 = ifelse(testData$V35 == "2", 1, 0)
    # Model
    gbmTrainModel <- gbm(V35~ ., data=trainingData, dist="bernoulli", n.tree = 400,shrinkage = 1, train.fraction = 1)
    # Predict
    gbmPredict=predict(gbmTrainModel,testData,n.trees = 400)
    # Accuracy
    gbmTable <- table(testData$V35>0, gbmPredict>0)
