...
# Read from URL
dataURL<-as.character('http://archive.ics.uci.edu/ml/machine-learning-databases/00225/Indian%20Liver%20Patient%20Dataset%20(ILPD).csv')
...
# Class attribute is column 11 ie V11
...
for(i in 1:10) 
{
    ...
    method <- "kNN"
    # Model
    knn.model <- kknn(formula = formula(V11~.),train=trainingData,test=testData,k = 3,distance=1)
    # Predict
    fit <- fitted(knn.model)
    tab3<-table(testData$V11, fit)
    ...
    bagTrainModel <- bagging(V11~., data=trainingData, control=control, nbagg=10, coob=TRUE)
    # Predict
    bagPredict <-predict(bagTrainModel, testData)
    # Accuracy
    baggingTable <- table(testData$V11, bagPredict)
    ...
    # Model 
    rFTrainmodel <- randomForest(as.factor(V11) ~ ., data=trainingData, ntree=500, keep.forest=TRUE, importance=TRUE)
    # Predict
    rFPredict <- predict(rFTrainmodel,testData)
    # Accuracy
    rFTable <- table(testData$V11, rFPredict)
    ...
    method <- "Boosting"
    # Model
    boostingTrainModel <- ada(V11 ~ ., data = trainingData, type="discrete")
    # Predict
    boostingPredict=predict(boostingTrainModel,testData)
    # Accuracy
    boostingTable <- table(testData$V11, boostingPredict)
    ...
    method <- "GBM Boosting"
    # Model
    trainingData$V11 = ifelse(trainingData$V11 == "2", 1, 0)
    testData$V11 = ifelse(testData$V11 == "2", 1, 0)
    # Model
    gbmTrainModel <- gbm(V11~ ., data=trainingData, dist="bernoulli", n.tree = 400,shrinkage = 1, train.fraction = 1)
    # Predict
    gbmPredict=predict(gbmTrainModel,testData,n.trees = 400)
    # Accuracy
    gbmTable <- table(testData$V11>0, gbmPredict>0)
