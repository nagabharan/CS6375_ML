...
for(i in 1:10) 
{
    ...
    # Model
    dtmodel <- rpart(as.factor(V2) ~ ., data = trainingData, parms=list(split="information"), method ="class", minsplit=1)
    ...
    method <-"SVM"
    # Model
    svmmodel <- svm(as.factor(V2) ~ ., data = trainingData, method="C-classification")
    # Predict
    ...
    method <-"Naive Bayes"
    # Model
    nbmodel <- naiveBayes(as.factor(V2) ~ ., data = trainingData,laplace=5)
    # Prediction
    ...
    method <-"Neural Net"
    # Model
    nnetmodel <- nnet(as.integer(V2)~., trainingData,size=1)
    # Predict
    pred <- predict(nnetmodel, testData)
    # Accuracy
    neural <- table(pred,testData$V2)
...
cat("\n")
cat("\nOutput for Breast Cancer Diagnostic  Dataset: ",dataURL)
