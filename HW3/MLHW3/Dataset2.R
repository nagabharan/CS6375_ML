...
for(i in 1:10) 
{
    ...
    # Model
    dtmodel <- rpart(as.factor(V14) ~ ., data = trainingData, parms=list(split="information"), method ="class", minsplit=1)
    prunedTree <- prune(dtmodel, cp =0.03)
    # Predict
    dTPredict <- predict(prunedTree, newdata = testData, type = "class")
    # Accuracy
    dTTable <- table(testData$V14, dTPredict)
    ...
    method <-"SVM"
    # Model
    svmmodel <- svm(as.factor(V14) ~ ., data = trainingData, method="C-classification")
    # Predict
    sVMPredict <- predict(svmmodel, testData,type = "class")
    # Accuracy
    sVMTable <- table(testData$V14, sVMPredict)
    ...
    method <-"Naive Bayes"
    # Model
    nbmodel <- naiveBayes(as.factor(V14) ~ ., data = trainingData,laplace=5)
    # Prediction
    nBPredict<- predict(nbmodel,testData)
    # Accuracy
    nBTable <- table(testData$V14, nBPredict)
    ...
    method <-"Neural Net"
    # Model
    nnetmodel <- nnet(as.integer(V14)~., trainingData,size=1)
    # Predict
    pred <- predict(nnetmodel, testData)
    # Accuracy
    neural <- table(pred,testData$V14)
...
cat("\n")
cat("\nOutput for Heart Disease Cleveland Processed Dataset: ",dataURL)
