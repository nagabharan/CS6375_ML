...
for(i in 1:10) 
{
    ...
    method <- "kNN"
    # Model
    knn.model <- kknn(formula = formula(V1~.),train=trainingData,test=testData,k = 3,distance=1)
    # Predict
    fit <- fitted(knn.model)
    tab3<-table(testData$V1, fit)
    ...
    bagTrainModel <- bagging(V1~., data=trainingData, control=control, nbagg=10, coob=TRUE)
    # Predict
    bagPredict <-predict(bagTrainModel, testData)
    # Accuracy
    baggingTable <- table(testData$V1, bagPredict)
    ...
    # Model 
    rFTrainmodel <- randomForest(as.factor(V1) ~ ., data=trainingData, ntree=500, keep.forest=TRUE, importance=TRUE)
    # Predict
    rFPredict <- predict(rFTrainmodel,testData)
    # Accuracy
    rFTable <- table(testData$V1, rFPredict)
    ...
    method <- "Boosting"
    # Model
    boostingTrainModel <- maboost(V1~., data = trainingData,iter=50,nu=2,maxdepth=6)
    # Predict
    boostingPredict=predict(boostingTrainModel,testData)
    # Accuracy
    boostingTable <- table(testData$V1, boostingPredict)
    ...
    method <- "GBM Boosting"
    # Model
    trainingData$V1 = ifelse(trainingData$V1 == "2", 1, 0)
    testData$V1 = ifelse(testData$V1 == "2", 1, 0)
    # Model
    gbmTrainModel <- gbm(V1~ ., data=trainingData, dist="bernoulli", n.tree = 400,shrinkage = 1, train.fraction = 1)
    # Predict
    gbmPredict=predict(gbmTrainModel,testData,n.trees = 400)
    # Accuracy
    gbmTable <- table(testData$V1>0, gbmPredict>0)
